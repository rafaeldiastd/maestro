...
const themeClasses = computed(() => {
    switch (subtype.value) {
        ...
        default: return 'border-lumina-border bg-lumina-card'
    }
})
...
const titleColor = computed(() => {
    switch (subtype.value) {
        ...
        default: return 'text-lumina-text'
    }
})
...
<template>
  <div class="flex flex-col items-center justify-center w-full mt-1 select-none">
      ...
      <!-- Properties (if any) -->
      <div v-if="card.properties && Object.keys(card.properties).length > 0" class="flex flex-wrap items-center justify-center gap-x-4 gap-y-1 mb-3 text-[10px] text-lumina-text-muted font-mono border-y border-lumina-border/30 py-1 w-full">
          <div v-for="(val, key) in card.properties" :key="key" class="flex gap-1">
              <span class="uppercase opacity-60">{{ key }}:</span>
              <span class="text-lumina-text opacity-90">{{ val }}</span>
          </div>
      </div>

      <!-- Body -->
      <div class="text-sm text-lumina-text leading-relaxed whitespace-pre-wrap text-center opacity-90 px-2 font-sans">
          {{ card.description }}
      </div>
  </div>
</template>
