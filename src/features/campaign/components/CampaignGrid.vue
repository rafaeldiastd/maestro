...
const getCardClasses = (asset) => {
   let classes = ''
   if (asset.is_revealed) classes += 'border-green-500/50 bg-green-500/5 '
   if (props.isPicker) classes += 'hover:ring-2 hover:ring-lumina-detail cursor-pointer'
   return classes
}
...
<template>
   <div
      ...
      @dragover.prevent @drop.prevent="$emit('file-drop', $event)">

      <!-- Empty State -->
      <div v-if="assets.length === 0 && folders.length === 0"
         class="flex h-full flex-col items-center justify-center text-lumina-text-muted pointer-events-none">
      ...
      <div v-else :class="gridClasses" class="grid gap-3 pb-20">
         ...
         <div v-for="asset in assets" :key="asset.id"
            ...
            :class="getCardClasses(asset)" draggable="true" @dragstart="handleDragStart($event, asset)"
            ...
            <div class="aspect-square w-full overflow-hidden bg-muted relative">
               <img :src="getAssetUrl(asset)" loading="lazy"
                  class="h-full w-full object-cover transition-all duration-500 group-hover:scale-105" :class="{
                     'blur-md': blurMode,
                     'opacity-50 grayscale': !asset.is_revealed
                  }">
               <div class="absolute inset-0 bg-background/0 group-hover:bg-background/20 transition-colors"></div>
            </div>

            <!-- Footer / Actions -->
            <div
               class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-background/90 via-background/50 to-transparent p-3 opacity-0 transition-opacity group-hover:opacity-100 flex items-end justify-between pt-8">

               <p class="truncate text-[10px] font-medium text-foreground max-w-[70%] drop-shadow-sm font-mono">
            ...
            <button @click.stop="$emit('delete', asset)"
               class="absolute top-2 left-2 p-1 rounded bg-background/50 text-foreground opacity-0 group-hover:opacity-100 hover:bg-destructive/90 hover:text-destructive-foreground transition-all backdrop-blur-sm z-20">
               <Trash2 class="h-3 w-3" />
            </button>
