  ...
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between pb-3 border-b border-lumina-border">
      <div class="flex items-center gap-2">
        ...
        <span class="text-xs uppercase tracking-wider font-medium text-lumina-text-muted">Connected Players</span>
      </div>
    ...
    <div v-if="players.length > 0" class="space-y-2">
      <div 
        ...
        <div class="flex items-center gap-3">
          <div class="relative">
            <!-- Avatar / Initials -->
            <div class="h-10 w-10 rounded-full bg-lumina-detail/10 border border-lumina-detail/20 flex items-center justify-center text-sm font-bold text-lumina-detail overflow-hidden">
          ...
          <div>
            ...
            <div class="flex items-center gap-2">
               ...
               <span v-if="player.role === 'master'" class="text-[10px] px-1.5 py-0.5 bg-lumina-detail/10 text-lumina-detail rounded border border-lumina-detail/20">DM</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-else class="text-center py-12">
      <div class="inline-flex h-12 w-12 items-center justify-center rounded-full bg-lumina-bg border border-lumina-border text-lumina-text-muted mb-3">
        <Users class="h-6 w-6" />
      </div>
      <p class="text-sm text-lumina-text-muted">No players connected yet</p>
    </div>
  </div>
</template>
