<template>
   <header class="flex h-12 items-center justify-between border-b border-lumina-border px-4 bg-lumina-bg shrink-0">
      ...
      <div class="flex items-center gap-3">
         <Button variant="ghost" size="icon-sm" class="md:hidden text-lumina-text-muted hover:text-lumina-text"
      ...
      <div class="flex items-center gap-1.5">
         ...
         <div class="flex items-center bg-lumina-card rounded-lg p-0.5 border border-lumina-border">
            <Button :variant="viewMode === 'grid' ? 'secondary' : 'ghost'" size="icon-sm" class="h-7 w-7 rounded-md"
               :class="viewMode === 'grid' ? 'bg-lumina-bg shadow-sm text-lumina-text' : 'text-lumina-text-muted hover:text-lumina-text'"
               @click="$emit('update:viewMode', 'grid')" title="Visualização em Grade">
               <LayoutGrid class="h-3.5 w-3.5" />
            </Button>
            <Button :variant="viewMode === 'small' ? 'secondary' : 'ghost'" size="icon-sm" class="h-7 w-7 rounded-md"
               :class="viewMode === 'small' ? 'bg-lumina-bg shadow-sm text-lumina-text' : 'text-lumina-text-muted hover:text-lumina-text'"
               @click="$emit('update:viewMode', 'small')" title="Visualização Compacta">
         ...
         <!-- Blur Toggle -->
         <Button :variant="blurMode ? 'warning' : 'ghost'" size="sm" class="h-7 text-xs font-medium gap-1.5"
            ...
            <span>{{ blurMode ? 'Oculto' : 'Visível' }}</span>
         </Button>
