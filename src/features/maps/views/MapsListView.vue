  ...
  <div class="h-screen w-screen bg-lumina-bg flex flex-col">
    ...
    <div class="h-14 border-b border-lumina-border bg-lumina-card flex items-center justify-between px-6 shrink-0">
      <div class="flex items-center gap-3">
        ...
        <h1 class="text-lg font-bold font-inknut text-lumina-text">Editor de Mapas</h1>
      </div>
    ...
    <div class="flex-1 overflow-y-auto p-6">
      <div v-if="maps.length === 0" class="flex items-center justify-center h-full">
        <div class="text-center">
          ...
          <p class="text-lg font-inknut text-lumina-text mb-2">Nenhum Mapa Ainda</p>
          <p class="text-sm text-lumina-text-muted mb-4">Crie seu primeiro mapa para começar</p>
          <Button @click="createMap" size="sm">Criar Mapa</Button>
        </div>
      </div>
        ...
        <div v-for="map in maps" :key="map.id"
          ...
          <div class="aspect-video bg-lumina-bg relative overflow-hidden">
            ...
            <div
              ...
              <span class="text-white font-bold">Editar Mapa</span>
            </div>
          </div>
          ...
          <div class="p-4">
            ...
            <p class="text-xs text-lumina-text-muted">
              {{ map.width }}×{{ map.height }}px · {{ map.grid_cell_width }}px grade
            </p>
          </div>

          <!-- Actions -->
          <div class="absolute top-2 left-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
            <Button @click.stop="toggleActive(map)" variant="ghost" size="sm"
              class="h-7 w-7 p-0 bg-lumina-card/90 hover:bg-lumina-bg" :title="map.is_active ? 'Desativar' : 'Ativar'">
              <svg v-if="!map.is_active" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24"
...
const deleteMap = async (map) => {
  if (!confirm(`Excluir "${map.name}"? Isso não pode ser desfeito.`)) return
