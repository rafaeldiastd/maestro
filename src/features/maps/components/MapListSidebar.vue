<template>
  <div class="fixed left-16 top-0 bottom-0 z-30 w-80 bg-card border-r border-border shadow-2xl">
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-2">
        <div v-for="map in maps" :key="map.id"
          ...
          <div class="absolute top-2 left-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
            <Button @click.stop="$emit('activate-map', map)" variant="ghost" size="sm"
            ...
            <Button @click.stop="$emit('delete-map', map)" variant="ghost" size="sm"
              class="h-7 w-7 p-0 bg-lumina-card/90 hover:bg-red-500/20 text-red-400" title="Delete">
