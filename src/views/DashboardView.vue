    ...
    <div class="min-h-screen w-full bg-lumina-bg text-lumina-text flex flex-col">
        ...
        <nav
            ...
            <div class="flex items-center gap-2">
                <div
                    class="h-8 w-8 rounded bg-gradient-to-br from-stone-700 to-black flex items-center justify-center font-bold text-white">
                    L</div>
        ...
        <main class="flex-1 container mx-auto p-6 max-w-5xl">
            ...
            <div class="flex items-center justify-between mb-8">
                ...
                <button @click="showNewSessionModal = true"
                    class="rounded-lg bg-stone-100 px-4 py-2 text-stone-900 font-semibold hover:bg-white transition-all shadow-lg shadow-stone-500/10 flex items-center gap-2">
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div v-for="session in sessions" :key="session.id" @click="enterSession(session.id)"
                    class="group relative h-48 rounded-xl bg-slate-800 border border-slate-700 p-6 flex flex-col justify-between hover:border-stone-400 hover:shadow-xl hover:shadow-stone-500/5 cursor-pointer transition-all">
                    <div>
                        <div class="flex items-start justify-between">
                            <h3 class="text-xl font-bold text-white group-hover:text-stone-200 transition-colors">{{
                                session.name }}</h3>
                            <span
                    ...
                    <div class="flex items-center justify-between mt-4">
                        ...
                        <span
                            class="text-sm font-medium text-stone-400 group-hover:text-white group-hover:translate-x-1 transition-transform flex items-center gap-1">
                            Launch
                ...
                <form @submit.prevent="createSession">
                    ...
                    <div class="flex items-center justify-end gap-3">
                        ...
                        <button type="submit" :disabled="creating"
                            class="rounded bg-stone-100 px-4 py-2 text-sm font-medium text-stone-900 hover:bg-white disabled:opacity-50">
